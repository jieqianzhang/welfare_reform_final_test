...
(function(factory) {
  ...
  lib.Parent = function(id, url, config) {
    ...
    this.sendMessage = function(messageType, message) {
      this.el.getElementsByTagName('iframe')[0].contentWindow.postMessage(
  ...
  lib.Child = function(config) {
    ...
    this.sendMessage = function(messageType, message) {
       ...
       */
      window.parent.postMessage(_makeMessage(this.id, messageType,
        message), '*');
    };
    ...
    this.messageRegex = new RegExp('^pym' + MESSAGE_DELIMITER + this.id +
      MESSAGE_DELIMITER + '(\\S+)' + MESSAGE_DELIMITER + '(.+)$');
...
function percentHeight() {
  ...
  // vertical height of viewport
  var viewHeight = Math.max(document.documentElement.clientHeight, window.innerHeight);

  var distanceTraveled = viewHeight - top;
